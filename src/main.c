...
float paddle_move(float paddle_position_y, float paddle_height, float *paddle_vel_y)
{
    if ((paddle_position_y + paddle_height >= height && *paddle_vel_y > 0) ||
        (paddle_position_y <= 0 && *paddle_vel_y < 0))
    {
        *paddle_vel_y = -1 * (*paddle_vel_y);
    }
    return paddle_position_y + *paddle_vel_y;
}
...
int main()
{
    ...
    while (!WindowShouldClose())
    {
        ...
        paddle_position[0].y = paddle_move(paddle_position[0].y, paddle_size.y, &paddle_vel_y);
        paddle_position[1].y = paddle_move(paddle_position[1].y, paddle_size.y, &paddle_vel_y);
        EndDrawing();
    }
